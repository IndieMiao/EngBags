...
function EngInventory_Update_BagSlots()
	if EngInventoryConfig["hide_bags_bar"] == 0 then
		...
		Ch4racterBagSlot1:Hide();
		Ch4racterBagSlot2:Hide();
		Ch4racterBagSlot3:Hide();
		Ch4racterBagSlot4:Hide();
	end
end
...
function EngPaperDollItemSlotButton_OnLoad()
	...
	for i=1,4,1 do
		...
		id, textureName, checkRelic = GetInventoryItemLink("player",invID);
	end
...
function EI_SetDefault(varname, defaultvalue, resetversion, cleanupfunction, cleanup_param1, cleanup_param2)
	...
	if (resetversion == nil) then
		-- more debugging
		EngInventory_PrintDEBUG("* Warning, EngInventory EI_SetDefault called with nil reset version: "..varname.." *");
		resetversion = 0;
	end
...
function EngInventory_SetDefaultValues(re)
	...
	EI_SetDefault("hide_bags_bar", 0, 1+re, EngBags_NumericRange, 0, 1);
	...
	for key,value in EngInventoryConfig["item_search_list"] do
		if (EngInventoryConfig["putinslot--"..value[1]] == nil) then
			EngInventory_PrintDEBUG("EngInventory: Unassigned catagory: "..value[1].." -- It has been assigned to slot 1");
...
function EngInventory_init()
	...
	elseif ( ERR_BADATTACKPOS == ERR_BADATTACKPOS_LOCAL_DE ) then
		...
		EngInventory_PrintDEBUG("EngInventory: No localization found, stuff won't work properly");
		EngInventory_load_Localization("EN");
	end
